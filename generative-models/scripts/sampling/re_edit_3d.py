...
def re_edit_3d(ori_img,ori_mask,edit_param,inp_cur,sv3d_model,sv3d_filter):
    edit_config_3D = {
        'elevations_deg': 10,  # default
        'azimuths_deg': generate_azimuth_angles(n_views_sv3d=21, angle_list_3d=angle_list_3d)
    }
    img_list, side_len = sv3d_sample(version='sv3d_p', decoding_t=5, elevations_deg=elevations_deg,
                                     azimuths_deg=azimuths_deg, input_img=ori_img, obj_mask=ori_mask,
    ...
    for edit_index, angle_select in enumerate(angle_list_3d):
        ...
        trans_img = img_list[idx][0]
        trans_img = cv2.resize(trans_img, (side_len, side_len))
        coarse_edit_res, transfered_mask = pasted_sv3d_back_to_img(ori_img, ori_mask, inp_cur, trans_img,
                                                                   constrain_area)
...
def main_2(data_id, base_dir):

    dst_dir_path_gen = osp.join(base_dir, f"coarse_edit_3d_ours")
    ...
    for da_n, da in tqdm(process_data.items(), desc=f'Proceeding repainting ours inference)'):
        ...
        for ins_id, current_ins in instances.items():
            ...
            for edit_ins, coarse_input_pack in current_ins.items():
                if osp.exists(get_save_path_edit(dst_dir_path_gen, da_n, ins_id, edit_ins)):
                ...
                inp_back_ground = cv2.cvtColor(
                    cv2.imread(osp.join(base_dir, f'inp_img_blended/{da_n}/{ins_id}/inp_img.png')),
                    cv2.COLOR_BGR2RGB)  # bgr
                # save_img_cr = Image.fromarray(coarse_input)
                # save_img_cr.save("new_edit_cr.png")

                coarse_input, target_mask = re_edit_3d(ori_img, ori_mask, edit_param, inp_back_ground)
                coarse_input_path = save_img(coarse_input, dst_dir_path_gen, da_n, ins_id, edit_ins)



def main(data_id, base_dir,use_3d):
    dst_base = osp.join(base_dir, f'Subset_{data_id}')
    if '200K' in dst_base:
        data_name = 'subject200K'
    ...
    if not use_3d:
        ...
        if osp.exists(osp.join(dst_base,f"3d_coarse_input_full_pack_{data_id}.json")):
            print(f'coarse edit for {data_id} already finish!')
            return
        if osp.exists(osp.join(dst_base,f"3d_temp_file_coarse.json")): #resume
            new_data = load_json(osp.join(dst_base,f"3d_temp_file_coarse.json"))
        else:
            new_data = dict()
        dataset_json_file = osp.join(dst_base, f"mat_fooocus_inpainting_{data_id}.json")
        dst_coarse_inp_path = osp.join(dst_base, "coarse_input_3d")

        dst_target_msk_path = osp.join(dst_base, "target_mask_3d")
    ...
    for da_n, da in tqdm(data.items(), desc=f'Proceeding coarse editing (Part {data_id})'):
        ...
        new_data[da_n]['4v_caption'] = caption
        ...
        # assert len(mask_list)==len(exp_mask_list) ,f'some error in length, da_n = {da_n} '
        #get instances
        #get img mask exp mask -> tgt mask, coarse input

        # sample_nums = 8
    #     edit_config = dict()
    #     edit_config['class_ratio']  = {
    #     "move": 0.35,
    #     "rotate": 0.35,
    #     "enlarge": 0.1,
    #     "shrink": 0.1,
    ...
    # }
        ...
        #resize img
        #rm bg
        constrain_areas = get_constrain_areas(mask_list)
        # constrain_areas_strict = get_constrain_areas(exp_mask_list)
        for ins_id in range(len(inp_img_list)):#ins_id
            sample_dict = dict()
            # mask_path, inp_path, exp_mask_path, obj_label = mask_list[ins_id], inp_img_list[ins_id], exp_mask_list[
            #     ins_id], obj_label_list[ins_id]
            ...
            constrain_areas = cv2.resize(constrain_areas, (h, w),interpolation=cv2.INTER_NEAREST)
            ...
            edit_prompt_list,edit_param_list,coarse_res_list,tgt_mask_list,out_of_img_boundary_list= coarse_edit_func_v2(img,mask_cur,inp_cur,cons_area,obj_label,sv3d_model=model,sv3d_filter=filter,use_3d=use_3d)
            ...
            for sample_id,edit_prompt in enumerate(edit_prompt_list):#sample_id
                ...
                out_of_img_boundary_stat = out_of_img_boundary_list[sample_id]
                ...
                per_edit_data['out_of_img_boundary'] = out_of_img_boundary_stat
            ...
            if not use_3d:
                ...
                save_json(new_data, osp.join(dst_base, f"3d_temp_file_coarse.json"))
    if not use_3d:
        ...
        save_json(new_data, osp.join(dst_base, f"3d_coarse_input_full_pack_{data_id}.json"))
        # remove temp file
        os.remove(osp.join(dst_base, f"3d_temp_file_coarse.json"))
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--use_3d', action='store_true', help="Flag to indicate if 3D processing should be used")
