...
def get_save_path_inp(base_dir, da_n, ins_n,method_name,blending):
    if not blending:
        inp_img_dir = os.path.join(base_dir, f'inp_img_no_blend_{method_name}')
    else:
        inp_img_dir = os.path.join(base_dir, f'inp_img_blended_{method_name}')
    inp_img_dir = os.path.join(base_dir, f'inp_img_no_blend_{method_name}')
    ...
    return final_path
def main(base_dir,blending,method_name):
    if not blending:
        inp_img_dir = os.path.join(base_dir, f'inp_img_no_blend_{method_name}')
    else:
        inp_img_dir = os.path.join(base_dir, f'inp_img_blended_{method_name}')
    ...
    for da_n, da in tqdm(data.items(),desc='proceeding reggio inpainting'):
        ...
        for ins_n, inst in instances.items():
            ...
            if osp.exists(get_save_path_inp(base_dir, da_n, ins_n,method_name,blending)):
            ...
            for edit_n, coarse_input_pack in inst.items():
                ...
                generated_results = model.Reggio_background_generation(ori_img, dil_ori_mask, "empty scene", guidance_scale=7.5,
                                                                   ...
                                                                   start_step=1, share_attn=True,method_type=method_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--blending', action='store_true', default=True, help="Enable blending functionality")
    args = parser.parse_args()



    #
    #USE METHOD NAME FOR ABLATION

    # 在需要时使用 device
    # model.to(device)
    # tensor = tensor.to(device)

    main(args.base_dir,args.blending,args.method_name)
