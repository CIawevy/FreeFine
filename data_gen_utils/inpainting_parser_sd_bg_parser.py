...
def get_save_path_inp(inp_img_dir, da_n, ins_n):
...
def main(base_dir,data_id):
    ...
    inp_img_dir = os.path.join(base_dir, 'inp_img_sd')
    ...
    # new_data = dict()
    device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
    for da_n, da in tqdm(process_data.items(), desc='proceeding sd inpainting bg'):
        instances = da['instances']
        for ins_n, inst in instances.items():
            ins_already_paint_stat = False
            if osp.exists(get_save_path_inp(inp_img_dir, da_n, ins_n)):
            ...
            for edit_n, coarse_input_pack in inst.items():
                ...
                inpainted_image = sd_inpainter(prompt="empty scene", image=to_inpaint_img, mask_image=repair_mask).images[0]
