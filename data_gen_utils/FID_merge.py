...
def merge_and_copy_json_data_fid(json_path_list,dest_dir):
    error_num = 0
    img_num = 0
    # 创建指定文件夹
    source_img_dir = os.path.join(dest_dir, 'source_img_full')
    ...
    # 遍历json文件路径列表
    for json_path in tqdm(json_path_list, desc=f'proceeding json files'):
        # 读取 json 文件
        if 'PIE' in json_path:
            type='PIE'
        ...
        # 遍历每个实例
        for img_id, instances in data.items():
            img_path = instances['src_img_path']
            final_path = os.path.join(source_img_dir, f"{img_num}.png")
            shutil.copy(img_path, final_path)
            img_num+=1
...
def merge_and_copy_json_data_fid_v2(json_path_list,dest_dir):
    error_num = 0
    img_num = 0
    # 创建指定文件夹
    source_img_dir = os.path.join(dest_dir, 'source_img_full_v2')
...
# json_path_list = [f"/data/Hszhu/dataset/PIE-Bench_v1/Subset_{i}/mask_tag_relabelled_lmm_v2_{i}.json" for i in range(4)]
# json_path_list_2 = [f"/data/Hszhu/dataset/Subjects200K/Subset_{i}/mask_tag_relabelled_lmm_v2_{i}.json" for i in range(4)]
# json_path_full = json_path_list + json_path_list_2
json_path_list = ["/data/Hszhu/dataset/PIE-Bench_v1/meta_data.json"]
json_path_list_2 = ["/data/Hszhu/dataset/Subjects200K/meta_data_unique.json"]
json_path_full = json_path_list + json_path_list_2
dest_dir = "/data/Hszhu/dataset/Geo-Bench/"
#
...
# 调用函数
merge_and_copy_json_data_fid_v2(json_path_full,dest_dir)
