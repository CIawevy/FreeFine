...
def get_save_path_inp(base_dir, da_n, ins_n,blending):
    ...
    return final_path
def main(base_dir,blending,data_id):
    if not blending:
        inp_img_dir = os.path.join(base_dir, f'inp_img_no_blend')
    else:
        inp_img_dir = os.path.join(base_dir, f'inp_img_blended')
    ...
    if data_id is not None:
        ...
        process_data = data
    for da_n, da in tqdm(process_data.items(),desc='blending bg inpainting '):
        ...
        for ins_n, inst in instances.items():
            ...
            for edit_n, coarse_input_pack in inst.items():
                ...
                generated_results =  cv2.cvtColor(cv2.imread(osp.join(base_dir,f'inp_img_no_blend/{da_n}/{ins_n}/inp_img.png')),cv2.COLOR_BGR2RGB )  # bgr
...
if __name__ == "__main__":
    ...
    main(args.base_dir,True,args.data_id)
