...
def re_edit_3d(coarse_input_ori,ori_target_mask, inp_back_ground):
    ...
    return reblend_img,ori_target_mask[:,:,0
                       ]
def main(base_dir,data_id,type_3d=False):
    if '3D' in base_dir:
        type_3d = True
    diy=False
    ...
    gen_img_dir = os.path.join(base_dir, 'Gen_results_sd')
    os.makedirs(gen_img_dir, exist_ok=True)
    # data = load_json("/data/Hszhu/dataset/Geo-Bench-SC/annotations.json")
    data = load_json(osp.join(base_dir,"annotations.json"))
    ...
    for da_n ,da in tqdm(process_data.items(),desc='sd_inpainting'):
        ...
        for ins_n,inst in instances.items():
            ...
            for edit_n ,coarse_input_pack in inst.items():
                if osp.exists(get_save_path_edit(gen_img_dir, da_n, ins_n, edit_n)) and not diy :
                    print(f'skip {da_n}_{ins_n}_{edit_n} for already exist')
                    continue
                diy=False
                if diy:
                    da_n = '0' #27 102
                ...
                inp_back_ground = cv2.cvtColor(
                    cv2.imread(osp.join(base_dir, f'inp_img_sd/{da_n}/{ins_n}/inp_img.png')),
                ...
                forbit_area = forbit_area[:,:,0]
                dil_tgt_mask = dilate_mask(target_mask, dilation_factor)
                dil_tgt_mask = np.where(forbit_area, 0, dil_tgt_mask)
                refine_area = dil_tgt_mask - target_mask
                if diy:
                    temp_view_img(coarse_input)
                    temp_view(refine_area)
                ...
                repair_mask = Image.fromarray(refine_area)
                ...
                final_path = get_save_path_edit(gen_img_dir, da_n, ins_n, edit_n)
                generated_results.save(final_path)  # 保存为PNG格式（单通道
                print(f'save img to {final_path}')
...
#current version is for GeoBench ,without drawmask
if __name__ == "__main__":
    ...
    main(args.base_dir,args.data_id)
