...
def main(data_id, base_dir):
    ...
    # if osp.exists(osp.join(dst_base,f"3d_generated_dataset_full_pack_{data_id}.json")):
    #     print(f'repainting for {data_id} already finish!')
    #     return
    dataset_json_file = osp.join(dst_base,f"3d_coarse_input_full_pack_{data_id}.json")
    ...
    dst_dir_path_gen = osp.join(dst_base,"Gen_results_3d/")
    #dict(edit_prompt: coarse input ,ori_Img ,ori_mask,target_mask)
    if osp.exists(osp.join(dst_base, f"3d_temp_file_repaint.json")):  # resume
        new_data = load_json(osp.join(dst_base, f"3d_temp_file_repaint.json"))
    else:
        new_data = dict()
    ...
    for da_n, da in tqdm(data.items(), desc=f'Proceeding repainting (Part {data_id})'):
        ...
        save_json(new_data, osp.join(dst_base, f"3d_temp_file_repaint.json"))

    save_json(new_data,osp.join(dst_base,f"3d_generated_dataset_full_pack_{data_id}.json"))
    # remove temp file
    os.remove(osp.join(dst_base, f"3d_temp_file_repaint.json"))
