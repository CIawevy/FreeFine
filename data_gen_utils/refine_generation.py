...
dataset_json_file = 'json_path' #TODO :json with editing prompt , mask , coarse input, ori_img, #ori_mask
...
for da_n, da in tqdm(data.items(), desc='Proceeding inpainting (Full)'):
    ...
    for ins_id,current_ins in instances.items():
        for edit_prompt,coarse_input_pack in current_ins.items():
            ...
            generated_results = model.generated_refine_results(ori_img,ori_mask,coarse_input,target_mask)#add gen_res in input_pack
            gen_img_path = save_imgs([generated_results], dst_dir_path_gen, edit_prompt)
            coarse_input_pack['gen_img_path'] = gen_img_path
            current_ins[edit_prompt] = coarse_input_pack
        instances[ins_id] = current_ins
    data[da_n]['instances'] = instances

save_json(data,"/data/Hszhu/dataset/PIE-Bench_v1/generated_dataset_full_pack.json")
