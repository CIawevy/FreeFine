...
dest_dir = "/data/Hszhu/dataset/Geo-Bench-3D"
gen_img_dir = os.path.join(dest_dir, 'Gen_results_sd')
os.makedirs(gen_img_dir, exist_ok=True)
# data = load_json("/data/Hszhu/dataset/Geo-Bench-SC/annotations.json")
data = load_json("/data/Hszhu/dataset/Geo-Bench-3D/annotations.json")
# data = load_json("/data/Hszhu/dataset/PIE-Bench_v1/Subset_0/coarse_input_full_pack_0.json")
if osp.exists(osp.join(dest_dir, f"generated_results_sd.json")):  # resume
    new_data = load_json(osp.join(dest_dir, f"generated_results_sd.json"))
else:
    new_data = dict()
        ...
        for edit_n ,coarse_input_pack in inst.items():
            ...
            try:
                inp_back_ground = cv2.cvtColor(
                    cv2.imread(osp.join(dest_dir, f'inp_img_sd/{da_n}/{ins_n}/inp_img.png')),
                    cv2.COLOR_BGR2RGB)  # bgr

            except:
                path = osp.join(dest_dir, f'inp_img_sd/{da_n}/{ins_n}/inp_img.png')

                assert False, f'no inp_img in {path}'
