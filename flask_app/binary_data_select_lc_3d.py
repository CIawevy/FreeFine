...
def initialize_subset_data(subset_path, subset_id):
    ...
    """
    # 动态加载或创建数据文件
    data_path = os.path.join(subset_path, f"3d_generated_dataset_full_pack_{subset_id}.json")
    ori_data_stat_path = os.path.join(subset_path, "data_select_stat_3d.json")
    undo_stack_path = os.path.join(subset_path, "data_select_undo_stack_3d.json")
    new_data_path = os.path.join(subset_path, f"full_3d_edit_sample_{subset_id}.json")
...
def filter_instance_final(data, da_n, ins_n):
    ...
    for edit_id, meta in instances.items():
        ...
        edit_results.append(meta['gen_img_path'])
...
def inner_init(username, subset_id):
    ...
    if subset_id not in global_data:
        # 加载或初始化子集数据
        json_path = os.path.join(directory, f"3d_generated_dataset_full_pack_{subset_id}.json")
        ori_data_stat_path = os.path.join(directory, "data_select_stat_3d.json")
        undo_stack_path = os.path.join(directory, "data_select_undo_stack_3d.json")
        new_data_path = os.path.join(directory, f"full_3d_edit_sample_{subset_id}.json")
