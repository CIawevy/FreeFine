...
def initialize_subset_data(subset_path, subset_id):
    ...
    """
    # 动态加载或创建数据文件
    data_path = os.path.join(subset_path, f"coarse_input_full_pack_{subset_id}.json")
    ori_data_stat_path = os.path.join(subset_path, "data_select_stat.json")
    undo_stack_path = os.path.join(subset_path, "data_select_undo_stack.json")
    new_data_path = os.path.join(subset_path, f"full_2d_edit_sample_{subset_id}.json")
...
def select_instance_results(username, subset_id, da_n,ins_n):
    ...
    return render_template(
        'instance_filter_flatten_fast_local.html',
        username=username,
        subset_id=subset_id,
        ori_img=instance_info['ori_img'],
        mask_img=instance_info['mask_img'],
...
def inner_init(username, subset_id):
    ...
    if subset_id not in global_data:
        # 加载或初始化子集数据
        json_path = os.path.join(directory, f"coarse_input_full_pack_{subset_id}.json")
        ori_data_stat_path = os.path.join(directory, "data_select_stat.json")
        undo_stack_path = os.path.join(directory, "data_select_undo_stack.json")
        new_data_path = os.path.join(directory, f"full_2d_edit_sample_{subset_id}.json")
