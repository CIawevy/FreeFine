...
def initialize_subset_data(subset_path, subset_id):
    ...
    """
    # 动态加载或创建数据文件
    data_path = os.path.join(subset_path, f"structure_completion_{subset_id}.json")
    ori_data_stat_path = os.path.join(subset_path, "data_select_stat_sc.json")
    undo_stack_path = os.path.join(subset_path, "data_select_undo_stack_sc.json")
    new_data_path = os.path.join(subset_path, f"structure_completion_select_{subset_id}.json")
...
def filter_instance_final(data, da_n, ins_n):
    ...
    id_key = list(instances.keys())[0]
    src_img_path = instances[id_key]['src_img_path']
    ori_mask_path = instances[id_key]['ori_mask_path']
    replace_name = os.environ.get("DATASET_DIR")
    vis_mask_path = ori_mask_path.replace(f'{replace_name}', f'{replace_name}vis_temp/')
    edit_results=[]
    labels = []
    for edit_id, meta in instances.items():
        #avoid 3d here
        # if 'y-axis' not in meta['edit_prompt']:
        edit_results.append(meta['coarse_input_path'])
        labels.append(shorten_prompt(meta['edit_prompt'],meta['obj_label']))
    # print(f'length of edit results is {len(edit_results)}')
    # 返回数据
    return {
        'ori_img': src_img_path,  # 原图
        'mask_img': vis_mask_path,  #掩码图
...
def correct_path(path):
    ...
    # 如果文件路径包含原来的前缀，就进行替换
    if path.startswith(original_path_prefix):
        path = path.replace(original_path_prefix, new_url_prefix, 1)
    return path
# 动态加载图片的路由
# @app.route('/file/<path:filename>')
# def serve_image(filename):
...
def select_instance_results(username, subset_id, da_n,ins_n):
    ...
    if request.method == 'POST':
        ...
        if action == 'skip_image':
            ...
            select_ids = request.form.get('discard_ids')  # 虽然前端传递的是 discard_ids，但我们将其视为 select_ids
            select_ids = json.loads(select_ids)  # 解析JSON
            ...
            # 遍历选中的 select_ids，并将对应的数据加入到 new_data 中
            for select_id in select_ids:
                # 将 select_id 转换为整数并减1（假设前端传递的ID是从1开始的）
                index = str(int(select_id) - 1)

                # 如果 data 中存在对应的数据，则将其加入到 new_data 中
                if index in data[da_n]['instances'][ins_n]:
                    new_data[da_n]['instances'][ins_n][index] = data[da_n]['instances'][ins_n][index]
    ...
    return render_template(
        'instance_filter_flatten_fast_local_select.html',
        username=username,
        subset_id=subset_id,
        ori_img=instance_info['ori_img'],
        mask_img=instance_info['mask_img'],
...
def inner_init(username, subset_id):
    ...
    if subset_id not in global_data:
        ...
        json_path = os.path.join(directory, f"structure_completion_{subset_id}.json")
        ori_data_stat_path = os.path.join(directory, "data_select_stat_sc.json")
        undo_stack_path = os.path.join(directory, "data_select_undo_stack_sc.json")
        new_data_path = os.path.join(directory, f"structure_completion_select_{subset_id}.json")
...
if __name__ == '__main__':
    ...
    if '200K' in args.dir:
        replace_name = 'Subject200K/'
    elif 'PIE' in args.dir:
        replace_name = 'PIE-Bench_v1/'
    elif 'GRIT' in args.dir:
        replace_name = 'GRIT/'
    elif 'SA' in args.dir:
        replace_name = 'SA-1B/'
    elif 'CelebA' in args.dir:
        replace_name = 'CelebAMask-HQ/'
        # 设置环境变量
    os.environ["DATASET_DIR"] = args.dir  # 数据集路径传递为环境变量
    os.environ["REPLACE_NAME"] = replace_name  # 数据集路径传递为环境变量
