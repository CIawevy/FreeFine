...
# 设置循环次数，防止下载中断
NUM_ITERATIONS=10  # 根据需要调整循环次数

# 请将以下信息替换为您的 Hugging Face token 和本地路径
HF_TOKEN="your_huggingface_token" #IF NEEDED
LOCAL_DIR="your_local_path"
TARGET="CIawevy/GeoBenchMeta"
# 执行下载命令
for ((i = 1; i <= NUM_ITERATIONS; i++)); do
    echo "正在执行第 $i 次下载..."
    huggingface-cli download --repo-type dataset -token $HF_TOKEN --resume-download $TARGET --local-dir $LOCAL_DIR --local-dir-use-symlinks False --resume-download
done
