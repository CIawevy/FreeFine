...
def transform_depth(
        ...
        rot_angle: float = None, rot_axis: torch.Tensor = None, translation: torch.Tensor = None,scale_factor:torch.Tensor = None,
...
def transform_depth_pc(
    ...
    # 新的调用方式（带缩放）
    pts, mod_ids = transform_point_cloud_v2(
        points=pts.cpu().numpy(),
        axis=rot_axis.cpu().numpy(),
        angle_degrees=rot_angle,
        x=translation[0].item(),
        ...
        z=translation[2].item(),
        mask=fg_mask.cpu().numpy()[0, 0],
        sx=scale_factor[0].item(),
        sy=scale_factor[1].item(),
        sz=scale_factor[2].item(),
    )
...
def transform_point_cloud_v2(points, axis, angle_degrees, x, y, z, mask, sx=1.0, sy=1.0, sz=1.0):
    """
    Rotate point cloud around the centroid of points selected by the mask, and also support scaling.
    ...
    - mask: boolean array of shape (512, 512) indicating which pixels to consider for the centroid
    - sx, sy, sz: scaling factors for x, y, z axes respectively, default is 1.0

    Returns:
    - rotated_points: numpy array of shape (512, 512, 3)
    - modified_indices: flattened boolean array indicating modified points
    """
    mask = mask.astype(bool)
    modified_indices = mask.flatten()  # Flattened version of the mask to match the reshaped points
    ...
    # Apply scaling
    scaled_points = translated_points.copy()
    scaled_points[:, :, 0] *= sx
    scaled_points[:, :, 1] *= sy
    scaled_points[:, :, 2] *= sz

    # Flatten the scaled points
    flattened_points = scaled_points.reshape(-1, 3)
    ...
    # Reshape the points back to 512x512x3 and translate back to the original position
    rotated_points = rotated_points_flattened.reshape(512, 512, 3) + centroid + np.array([x, y, z])
