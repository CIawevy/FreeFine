...
class MultiModalDataset(Dataset):
    def __init__(self, data_dict, dst_dir_path_gen, check_exist=True, total_split=8, data_id=0):
        self.cases = []
        ...
        # 计算当前批次的数据范围
        total_items = len(self.cases)
        items_per_split = (total_items + total_split - 1) // total_split
        start_idx = data_id * items_per_split
        end_idx = min(start_idx + items_per_split, total_items)
        self.cases = self.cases[start_idx:end_idx]

        print(f" 批次 {data_id+1}/{total_split} 发现 {len(self.existing_results)} 个已有结果")
        print(f" 批次 {data_id+1}/{total_split} 待处理案例: {len(self.cases)}")
...
def main(dst_base,local_rank,data_id):
    # 初始化分布式环境
    ...
    dataset =MultiModalDataset(data,dst_dir_path_gen=dst_dir_path_gen,total_split=8,data_id=data_id)
    # 2. 创建分布式采样器
    ...
    for batch_idx, batch_case in enumerate(tqdm(dataloader, desc="Multi-GPU Inference")):
        ...
        prompt = ""
        dx, dy, _, _, ry, rz, sx, sy, _ = edit_param
        dx /= -512
        dy /= -512
        # rz = rz
        ...
        fg_mask = preprocess_image(mask,local_rank=local_rank)
        ...
        img = preprocess_image(image,local_rank=local_rank)
    ...
    #
    #     save_json(new_data, osp.join(base_dir, f"generated_results_diffusionhandles.json"))
...
if __name__ == "__main__":
    ...
    main(base_dir,args.gpu_id,args.data_id)