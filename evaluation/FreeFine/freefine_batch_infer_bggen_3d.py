...
def main(dst_base,blending):
    ...
    # 数据准备
    dataset_json = osp.join(dst_base, "annotations_3d.json")
    if not blending:
        inp_img_dir = os.path.join(base_dir, f'Geo-Bench-3D/inp_img_no_blend')
    else:
        inp_img_dir = os.path.join(base_dir, f'Geo-Bench-3D/inp_img_blended')
    ...
    for batch in tqdm(dataloader, desc=f"GPU {local_rank} Processing"):
        ...
        # can be index with GeoBenchMeta/Geo-Bench-3D/{INP_IMG_DIR}/{DA_N}/{INS_ID}/inp_img.png
