...
def main(method_name='caa'):
    dst_base = "/data/Hszhu/dataset/Geo-Bench-SC/"
    save_json_name = osp.join(dst_base, f"generated_results_ours_random.json")
    if osp.exists(save_json_name):
        print(f'results already finish!')
        return
    dst_dir_path_gen = osp.join(dst_base, f"Gen_results_ours_random/")
    ...
    temp_file_name = osp.join(dst_base, f"temp_file_repaint_{method_name}.json")
    if osp.exists(temp_file_name):  # resume
        new_data = load_json(temp_file_name)
    else:
        new_data = dict()

    for da_n, da in tqdm(data.items(), desc=f'Proceeding repainting ours inference)'):
        ...
        for ins_id,current_ins in instances.items():
            ...
            for edit_ins,coarse_input_pack in current_ins.items():
                ...
                # coar_img_path = "/data/Hszhu/dataset/vis/coarse_input/27/0/coarse_7.png"
                # coar_img_path = "/data/Hszhu/dataset/PIE-Bench_v1/Subset_0/coarse_input/6/0/8.png"
                # draw_mask_path = "/data/Hszhu/Reggio/data_gen_utils/obj_remove_mask.png"
                # draw_mask_path =  "/data/Hszhu/Reggio/draw_mask2.png"
                draw_mask_vis(coarse_input,draw_mask,f"/data/Hszhu/dataset/Geo-Bench-SC/draw_mask/{da_n}/{ins_id}/draw_{edit_ins}.png")
                ...
                new_inst[edit_ins] = coarse_input_pack
            instances[ins_id] = new_inst
        new_data[da_n] = dict()
        new_data[da_n]['instances'] = instances
        save_json(new_data, temp_file_name)
    save_json(new_data, save_json_name)
    # remove temp file
    os.remove(temp_file_name)

if __name__ == "__main__":
    # 使用 argparse 解析命令行参数
    parser = argparse.ArgumentParser(description="sc repainting processing script")
