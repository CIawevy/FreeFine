...
class DDIMInversion(object):
    ...
    def sample(self,
        ...
        time_range = timesteps      # Don't flip, b/c inversion
        total_steps = timesteps.shape[0]

        iterator = tqdm(time_range, desc='DDIM Inverter', total=total_steps)

        # FLIP ALL BECAUSE INVERSION
        #alphas = self.ddim_alphas
        #alphas_prev = self.ddim_alphas_prev
        #sqrt_one_minus_alphas = self.ddim_sqrt_one_minus_alphas
        #sigmas = self.ddim_sigmas
        alphas = torch.flip(self.ddim_alphas, (0,))
        alphas_prev = np.flip(self.ddim_alphas_prev)
        sqrt_one_minus_alphas = torch.flip(self.ddim_sqrt_one_minus_alphas, (0,))
        sigmas = torch.flip(self.ddim_sigmas, (0,))
        ...
        for i, step in enumerate(iterator):
            ...
            # Make estimate of noise, cond and uncond
            ...
            # Pred next (x_{t} from x_{t-1})
            # img = x_t
            with torch.no_grad():
                dir_xt = (1. - a_prev - sigma_t ** 2).sqrt() * e_t
                img = a_t.sqrt() / a_prev.sqrt() * (img - dir_xt) + sqrt_one_minus_at * e_t


            if operation.save_latents:
                save_path = Path(f'{operation.folder}/latents')
                save_path.mkdir(exist_ok=True)
                torch.save(img, save_path / f'zt.{i:05}.pth')
