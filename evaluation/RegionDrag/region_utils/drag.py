...
def copy_and_paste(source_latents, target_latents, source, target):
    t_s = source_latents.flatten()
    t_t = target_latents.flatten()
    count = 0
...
def drag(drag_data, steps, start_t, end_t, noise_scale, seed, progress=tqdm, method='Encode then CP', save_path=''):
    ...
    # s_n = source.to('cpu').numpy()
    # t_n = target.to('cpu').numpy()
    # m = np.zeros((64,64))
    # m[s_n[:,1],s_n[:,0]] = 255
    # Image.fromarray(m).convert("RGB").save("mask.png")

    # k = np.zeros((64,64))
    # k[t_n[:,1],t_n[:,0]] = 255
    # Image.fromarray(k).convert("RGB").save("mask_tgt.png")

   

    ##### The inputs are tuple, hardcode to get the value 
    ...
    if method in ('Encode then CP', 'CP then Encode'):
        ...
        with torch.no_grad():
            ...
            mask_pt = mask.unsqueeze(0).unsqueeze(0).to(device).to(torch.float32)
            mask_pt = F.interpolate(mask_pt, size=latent.shape[2:]).expand_as(latent)
