...
def calculate_md(data, image_label):
    ...
    for data_pair in tqdm(data_pairs):
        ...
        kps = kps[:max_points]
        t_coords_pixels = get_transform_coordinates(edit_param, s_img.shape[:-1], s_mask)
...
if __name__ == "__main__":
    from main import parse_data_level
    import json

    data = json.load(open("dragon.json"))
    data = parse_data_level(data, 1)
    calculate_md(data, "gen_img_path")
