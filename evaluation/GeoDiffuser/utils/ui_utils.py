...
def project_image_latest(input_img,
                    mask_image,
                    depth_image,
                    transform_mask,
                    transform_in,
    ...
    #TODO: change to vis code , donot use it for eval unless chang back to project_image
    #the new functions aims to get 1.coarse3d img with depthanything 2.coord_correspond 3.transformed mask 4.md
    use_softsplat = True

    t_coords, p_image, mask_projected_mesh = vis_utils.get_transform_coordinates(input_img / 255.0, depth_image, mask_image[..., 0], transform_in = transform_in, use_softsplat = use_softsplat, focal_length = focal_length, return_mesh=True)
    t_coords_depth = torch.tensor(t_coords)[None]
    ...
    # 不将 olive 混合到原始区域，直接进行图像拼接的逻辑
    # 之前的代码有 olive 相关的混合操作，现在去除以下与 olive 混合的代码
    ...
    # 直接进行图像拼接
    # 根据 t_mask_1 和 mask_projected_m 拼接 p_image 和 b_img
    # 这里的逻辑是根据掩码决定使用 p_image 还是 b_img 中的像素
    p_image_out = np.where(t_mask_1, p_image, b_img)
    ...
    return p_image_out,t_mask_1[:,:,0],mask_projected_mesh[0][0],t_coords[:,:,:2]
...
def get_transformed_mask(input_img, 
    ...
    return project_image_latest(input_img, mask_image, depth_image, transform_mask, transform_in, splatting_radius, background_img, splatting_tau = splatting_tau, splatting_points_per_pixel = splatting_points_per_pixel, focal_length=focal_length,)


def get_correspondence_and_coarse_img(input_img,
                         mask_image,
                         depth_image,
                         transform_mask,
                         translation_x,
    ...
    transform_in = transform_in @ vis_utils.translateMatrix(translation_x, translation_y, translation_z).type_as(
        transform_in)
    ...
    return project_image(input_img, mask_image, depth_image, transform_mask, transform_in, splatting_radius,
                         background_img, splatting_tau=splatting_tau,
                         splatting_points_per_pixel=splatting_points_per_pixel, focal_length=focal_length)
