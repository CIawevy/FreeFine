...
def temp_view_img(image: Image.Image, title: str = None) -> None:
    ...
    # Show the image
    plt.show()
def init_model(model_path, model_dtype="fp16", num_ddim_steps=50,device=None):
    if device is None:
        ...
        #TODO: tobe check
            ...
            theta = torch.tensor([
                ...
                [torch.sin(torch.deg2rad(torch.tensor(angle))),
                 torch.cos(torch.deg2rad(torch.tensor(angle))),
                 0]
            ], dtype=torch.float32).unsqueeze(0).to(mp.device)

            # 应用旋转
            grid = F.affine_grid(theta, mp_float32.size(), align_corners=False)
            new_mp = F.grid_sample(mp_float32, grid, mode='nearest', padding_mode='zeros', align_corners=False)

            # 转换回原始精度
            new_mp = new_mp.to(original_dtype)
        if flip is not None:
...
class LayerFusionSC:
    ...
    def mv_layer(self, x_t, bg_id, fg_id, op_id):
        bg_img = x_t[bg_id:(bg_id + 1)].clone()  # X[1]  #[Z0,ZBG,ZC,Z1] #[ZS,Z0,ZC,Z1]
        fg_img = x_t[fg_id:(fg_id + 1)].clone()
        fg_mask = self.fg_mask_list[fg_id - 3]
        op_list = self.op_list[fg_id - 3]
        ...
        for item in op_list:
            ...
            if scale != 0:
                fg_img = self.mv_op(fg_img, op=op, scale=scale)
                fg_mask = self.mv_op(fg_mask, op=op, scale=scale)
        # modified based on my understanding
        x_t[op_id:(op_id + 1)] = bg_img * (1 - fg_mask-draw_mask) + fg_img * fg_mask +x_t[op_id:(op_id + 1)]*draw_mask # ZC = M*Z1 + (1-m-md)*z0 + md*Zc
    ...
    def __call__(self, x_t):
        ...
        if self.blend_time[0] <= self.counter <= self.blend_time[1]:
            ...
            op_id = 2  # canvas
            for fg_id in range(3, b):  # fg_layer
                self.mv_layer(x_t, bg_id=bg_id, fg_id=fg_id, op_id=op_id) #modify for draw mask in this func
                bg_id = op_id

        return x_t

    def __init__(self, remove_mask, fg_mask_list, refine_mask=None,draw_mask=None,
                 blend_time=[0, 40],
                 mode="removal", op_list=None):
        self.counter = 0
        self.mode = mode
...
class DesignEdit():
    def __init__(self, pretrained_model_path="/home/jyr/model/stable-diffusion-xl-base-1.0",device=None):
        ...
        self.attend_scale = {}
        self.device = device
        self.ldm_model, self.inversion= init_model(model_path=self.pretrained_model_path, model_dtype=self.model_dtype, num_ddim_steps=self.num_ddim_steps,device=self.device)
    ...
    def run_remove(self, original_image=None, mask_1=None, mask_2=None, mask_3=None, refine_mask=None, 
        ...
        prompt="", save_dir="./tmp", mode='removal',use_proximal=True):
        # 01-1: 
    ...
    def process_layer_states(self, layer_states):
        ...
        for state in layer_states:
            img, mask, dx, dy, resize, rotate, w_flip, h_flip = state
            if img is not None:  
                ...
                if w_flip == "left/right" and h_flip == "down/up":
                    ...
                    flip_code = 0
                op_list.append([dx_command, dy_command,rz_command])
        ...
        sample_ref_match = {0: 0, 1: 0, 2: 0, 3: 1, 4: 2, 5: 3}#what does it mean
    ...
    def run_mask(self, mask_1, mask_2, mask_3, mask_4):
        ...
        return final_mask
    def time_testing(self, bg_img, bg_ori, bg_mask, l1_dx, l1_dy, l1_resize,l1_rotate,
    l1_w_flip=None, l1_h_flip=None, selected_points=None,
        prompt="", save_dir="./tmp", mode='layerwise'):
        # 00： prepare: layer-wise states
        bg_img = bg_ori if bg_ori is not None else bg_img
        ...
        if bg_mask is None:
            ...
            bg_mask = utils.convert_and_resize_mask(bg_mask)
        l1_mask = bg_mask
        l1_state = [l1_img, l1_mask, l1_dx, l1_dy, l1_resize, l1_rotate,l1_w_flip, l1_h_flip]
        ori_shape = bg_img.shape
        ...
        # return [resized_img]
        return 'ok'
    def infer_2d_edit(self, bg_img, bg_ori, bg_mask, l1_dx, l1_dy, l1_resize,l1_rotate,
    l1_w_flip=None, l1_h_flip=None, selected_points=None,
        prompt="", save_dir="./tmp", mode='layerwise'):
        # 00： prepare: layer-wise states
        bg_img = bg_ori if bg_ori is not None else bg_img
        ...
        resized_img = cv2.resize(images[2], (ori_shape[1], ori_shape[0]))
        return [resized_img]
    def infer_2d_edit_sc(self, bg_img, bg_ori, bg_mask,draw_mask, l1_dx, l1_dy, l1_resize,l1_rotate,
    l1_w_flip=None, l1_h_flip=None, selected_points=None,
        prompt="", save_dir="./tmp", mode='layerwise'):
        # 00： prepare: layer-wise states
        bg_img = bg_ori if bg_ori is not None else bg_img
        ...
        if draw_mask is None:
            draw_mask = np.zeros((1024, 1024), dtype=np.uint8)
        else:
            draw_mask = utils.convert_and_resize_mask(draw_mask)
        l1_mask = bg_mask
        l1_state = [l1_img, l1_mask, l1_dx, l1_dy, l1_resize, l1_rotate,l1_w_flip, l1_h_flip]
        ori_shape = bg_img.shape
        ...
        # 01-1: prepare: image_gt, remove_mask, fg_mask_list, refine_mask,draw_mask 128,128
        attend_scale = 20
        image_gt = [bg_img] + image_paths
        image_gt = [Image.fromarray(img).resize((1024, 1024)) for img in image_gt]
        image_gt = np.stack(image_gt)
        remove_mask = utils.attend_mask(bg_mask, attend_scale=attend_scale)
        refine_mask = None
        draw_mask = utils.attend_mask(draw_mask,attend_scale=0)#no dilation since draw mask is enough and thus fair
        ...
        _, x_t, x_stars, prompt_embeds, pooled_prompt_embeds = self.inversion.invert(image_gt, prompts, inv_batch_size=len(image_gt))
        # 03: init layer_fusion and controller
        lb = LayerFusionSC(remove_mask=remove_mask, fg_mask_list=fg_mask_list, blend_time=blend_time, refine_mask=refine_mask,draw_mask=draw_mask,
                    mode=mode, op_list=op_list)
        controller = Control(layer_fusion=lb)
        register_attention_control(model=self.ldm_model, controller=controller, mask_time=self.mask_time, refine_time=refine_time)
        # 04: generate images
