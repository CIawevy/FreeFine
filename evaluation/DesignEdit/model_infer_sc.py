    ...
    for da_n, da in tqdm(data.items(), desc=f'Proceeding editing'):
        ...
        for ins_id,current_ins in instances.items():
            ...
            for edit_ins,coarse_input_pack in current_ins.items():
                ...
                target_mask = read_and_resize_mask(coarse_input_pack['tgt_mask_path'])
                ...
                # draw_mask = np.zeros_like
                draw_mask = read_and_resize_mask(
                    f"/data/Hszhu/dataset/Geo-Bench-SC/draw_mask/{da_n}/{ins_id}/draw_{edit_ins}.png")
                #process draw_mask #512 512 3
                stat = (1-target_mask).astype(bool)
                non_overlap_draw_mask = np.where(1-target_mask,draw_mask,0)
                ...
                rz = -rz
                with torch.no_grad():
                    generated_results = model.infer_2d_edit_sc(
                        ori_img, ori_img, ori_mask,non_overlap_draw_mask, dx, dy, sx, rz,
                    )
