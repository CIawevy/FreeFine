...
def interpolate_feature_patch(feat,
                              ...
                              x1,
                              x2):
    x1_floor = torch.floor(x1).long().clip(min=0)
    x1_cell = x1_floor + 1
    dx = torch.floor(x2).long() - torch.floor(x1).long()

    y1_floor = torch.floor(y1).long().clip(min=0)
    y1_cell = y1_floor + 1
    ...
    try:
        ...
        print(f"Error in interpolate_feature_patch: {e}")
        print(f"Shape of Ia: {Ia.shape}, wa: {wa.shape}")
        print(f"Shape of Ib: {Ib.shape}, wb: {wb.shape}")
        print(f"Shape of Ic: {Ic.shape}, wc: {wc.shape}")
    ...
    for step_idx in range(args.n_pix_step):
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            for i in range(len(handle_points)):
                ...
                r1, r2 = math.floor(max(0,int(pi[0])-args.r_m)), math.floor(min(max_r,int(pi[0])+args.r_m+1))
                c1, c2 = math.floor(max(0,int(pi[1])-args.r_m)), math.floor(min(max_c,int(pi[1])+args.r_m+1))
                ...
                if f0_patch.shape[2] > f1_patch.shape[2]:
                    f0_patch = f0_patch[:,:,1:,:]
                elif f0_patch.shape[2] < f1_patch.shape[2]:
                    f1_patch = f1_patch[:,:,1:,:]
                elif f0_patch.shape[3] > f1_patch.shape[3]:
                    f0_patch = f0_patch[:,:,:,1:]
                elif f0_patch.shape[3] < f1_patch.shape[3]:
                    f1_patch = f1_patch[:,:,:,1:]
                else:
                ...
                try:
                    ...
                    import pdb; pdb.set_trace()
                    print(f"Error in drag_diffusion_update: {e}")
                    print(f"Shape of f0_patch: {f0_patch.shape}, f1_patch: {f1_patch.shape}")   
