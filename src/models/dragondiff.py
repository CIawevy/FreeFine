...
class DragonPipeline:
    def __init__(self, sd_id='stabilityai/stable-diffusion-2-1', ip_id='models/ip_sd15_64.bin', NUM_DDIM_STEPS=50, precision=torch.float32, ip_scale=0,device=None):
        self.device = device
        ...
        onestep_pipe.estimator = MyUNet2DConditionModel.from_pretrained(sd_id, subfolder="unet",vae=None, text_encoder=None, tokenizer=None,
                                ...
                                safety_checker=None, feature_extractor=None,).to(self.device, dtype=precision)
        ...
        self.ip_id = ip_id
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained("/data/Hszhu/prompt-to-prompt/IP-Adapter/", subfolder='models/image_encoder').to(self.device, dtype=precision)
    ...
    def init_proj(self, precision):
        image_proj_model = Resampler(
            ...
            ff_mult=4
        ).to(self.device, dtype=precision)
    ...
    def get_image_embeds(self, pil_image):
        ...
        clip_image = clip_image.to(self.device, dtype=self.precision)
    ...
    def load_adapter(self, model_path, scale=1.0):
        ...
        for name in self.pipe.unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                scale=scale,num_tokens= self.num_tokens).to(self.device, dtype=self.precision)
