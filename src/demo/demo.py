...
def create_my_demo_full_2D(runner):
    DESCRIPTION = """
    ## Baseline Demo with simple copy-paste and inpainting
    Usage:
    ...
    with gr.Blocks() as demo:
        ...
        clear_button.click(fn=fun_clear,
                           ...
                           outputs=[original_image, global_points, global_point_label, selected_points, mask,mask_ref,
                                    img_draw_box, img, output, output_edit , noised_img,INP_Mask, EXP_Mask,img_ref,EXP_Mask_2,retain_region ])
    return demo
def create_my_demo_full_2D_ctn(runner):
    DESCRIPTION = """
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
                ...
                with gr.Box():
                    ...
                    with gr.Box():
                        ...
                        lam = gr.Slider(
                            ...
                            value=0.1,
                            interactive=True)
                        motion_split_steps = gr.Slider(
                            label="motion_split_steps",
                            minimum=0,
                            maximum=100,
                            step=1,
                            value=20,
            ...
            with gr.Column():
                with gr.Box():
                    ...
                    gr.Markdown("<h5><center>OriGen</center></h5>")
                    output_reconstruct = gr.Gallery().style(grid=1, height='auto')

                    gr.Markdown("<h5><center>EditGen</center></h5>")
                    output_edit = gr.Gallery().style(grid=1, height='auto')

                    gr.Markdown("<h5><center>OptimizedLatent</center></h5>")
                    noised_img = gr.Gallery().style(grid=1, height='auto')

                    gr.Markdown("<h5><center>TargetMask</center></h5>")
                    target_Mask = gr.Gallery().style(grid=1, height='auto')
        ...
        run_button.click(fn=runner,
                         inputs=[original_image, mask, prompt,motion_split_steps, seed, selected_points, guidance_scale, num_step, max_resolution,dilate_kernel_size,max_times,sim_thr,lr,lam,
                                 start_step,end_step,mask_ref,eta,use_mask_expansion,standard_drawing,contrast_beta,resize_scale,rotation_angle,strong_inpaint,flip_horizontal,flip_vertical,cross_enhance,
                                 mask_threshold,mask_threshold_target,blending_alpha], outputs=[output_reconstruct ,output_edit ,noised_img ,target_Mask,])
        clear_button.click(fn=fun_clear,
                           inputs=[original_image, global_points, global_point_label, selected_points, mask,mask_ref,motion_split_steps,end_step,max_times,sim_thr,lr,lam,
                                   img_draw_box, img, output_reconstruct, output_edit, noised_img,img_ref,target_Mask],
                           outputs=[original_image, global_points, global_point_label, selected_points, mask,mask_ref,motion_split_steps,end_step,max_times,sim_thr,lr,lam,
                                    img_draw_box, img, output_reconstruct, output_edit , noised_img,target_Mask])
    return demo

def create_my_demo_full_3D_magic(runner):
    DESCRIPTION = """
    ## Baseline Demo with simple copy-paste and inpainting
    Usage:
    - Upload a source image, and then draw a box to generate the mask corresponding to the editing object(optional).
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                with gr.Box():
                    ...
                    img_ref = gr.Image(sourc='upload', tool="sketch", label="Original image", interactive=True,
                                       type="numpy")
        ...
        run_button.click(fn=runner,
                         inputs=[original_image, mask, prompt, INP_prompt, seed, guidance_scale, num_step,
    ...
    with gr.Blocks() as demo:
        ...
        run_button.click(fn=runner,
                         inputs=[original_image, mask, prompt, INP_prompt, seed, guidance_scale, num_step, max_resolution,mode,dilate_kernel_size,
                                 ...
                                 mask_threshold,mask_threshold_target,blending_alpha,splatting_radius,splatting_tau,splatting_points_per_pixel,focal_length], outputs=[output_edit ,INP_IMG ,INP_Mask,TGT_MSK, depth_map])
        clear_button.click(fn=fun_clear,
